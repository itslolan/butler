...
CREATE INDEX idx_transactions_needs_clarification ON transactions(needs_clarification) WHERE needs_clarification = true;

CREATE INDEX idx_user_metadata_user_id ON user_metadata(user_id);

-- Account snapshots table for monthly balance tracking
...
CREATE INDEX idx_snapshots_user_account ON account_snapshots(user_id, account_name);
